...
| `FRONTEND_URL`, `CORS_ORIGINS`, `PORT` | Configuration | No | ✅ Pre-set |
| `AUTO_SYNC_INTERVAL_SECONDS` | Configuration. Seconds between polling sweeps when Google refuses the webhook. `0` or unset = off. **Must stay `0` locally** while `backend/.env` points at the production database, or your laptop sorts production users' images. On DigitalOcean: `60` until Google accepts the webhook ([domainguide.md](domainguide.md)) | No | Optional |
