...
export function validateTemplate(template: string, tagKeys: string[] = []): string[] {
  const errors: string[] = [];
  const text = String(template ?? '');
 ...
 * tokens don't leave doubled separators behind.
 */
export function renderFileName(
  template: string,
  values: NameValues,
  { originalName, mimeType }: { originalName?: string | null; mimeType?: string | null } = {},
): string {
